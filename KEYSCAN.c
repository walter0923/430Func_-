...
void KeyProc(void) {
	...
	if (key == 0x00) {
		if ((LastKey == HEAD_UP) || (LastKey == FOOT_UP)
				|| (LastKey == HEAD_DOWN) || (LastKey == FOOT_DOWN)) {
			//send stop
			RfDataSend(HFOFF);
		}
...
__interrupt void TIMERA0_ISR(void) {
	...
	if ((HeadStepTC10us++) >= 100) {
		HeadStepTC10us = 0;
		HeadStepTC1ms++;
		if ((NowFunc == HEAD_UP) || (NowFunc == FOOT_UP)
				|| (NowFunc == HEAD_DOWN) || (NowFunc == FOOT_DOWN)
				|| (NowFunc == FLAT)) {
			NoMoveDetc();	//30ms no set not cnt shout down head & foor motor
		}
	}
